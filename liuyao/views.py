...
class PaipanResult(APIView):
    def post(self, request):
        ...
        return Response(self.get_gua(gua_image))
    ...
    def get_gua(self, guaImage):
        ...
        liuqin_set = Liuqin.objects.filter(liuqin="兄弟").first()
        ...
        # 获得卦的各爻地支
        bengua_neigua_dizhi = Huntianjiazi.objects.filter(gua=bengua_neigua.gua).filter(neiwai="内").first()
        bengua_waigua_dizhi = Huntianjiazi.objects.filter(gua=bengua_neigua.gua).filter(neiwai="外").first()
        biangua_neigua_dizhi = Huntianjiazi.objects.filter(gua=biangua_neigua.gua).filter(neiwai="内").first()
        biangua_waigua_dizhi = Huntianjiazi.objects.filter(gua=biangua_neigua.gua).filter(neiwai="外").first()
