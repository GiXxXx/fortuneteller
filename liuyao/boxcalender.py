...
#  Copyright 2014 ccdjh <ccdjh.marx@gmail.com>
...
class BoxCalender():
    def day(self, english_year, english_month, english_day):
        # 旧历的计算
        chinese_years_days = self.helper_days_list()
        ...
        if english_days < 50:
            d49 = self.day_49(english_year, english_month, english_day)
            return d49
        chinese_year_list = self.helper_year_data_find(english_days, chinese_years_days)  # 获得年位置 [33271, 91, 1992, 16]
        ...
        chinese_month_list = self.helper_month_data_find(chinese_year_list[3], d)  # 获得月位置 [0, 0, 1, 17]
        ...
        # 天干地支的计算
        chinese_years_days_60 = self.helper_days_60_list()
        chinese_year_list = self.helper_year_60_data_find(english_days, chinese_years_days_60)  # 获得年干支位置[1130, 3, 1904, 60]
        ...
        chinese_month_days_60 = self.helper_month_60_data_find(chinese_year_list[3], g_list_new)  # 获得月干支位置[60, 2, 3, 1]
        ...
        f3 = CHINESE_10_1[int(year_60_list[chinese_year_list[1]][0:2])] + CHINESE_12_1[
            int(year_60_list[chinese_year_list[1]][2:4])] + '-' + v2[chinese_month_days_60[1]] + v3[
                 chinese_month_days_60[1]] + '-' + CHINESE_10_1[int(CHINESE_60[v5][0:2])] + CHINESE_12_1[
                 int(CHINESE_60[v5][2:4])]
        f4 = CHINESE_12_2[int(year_60_list[chinese_year_list[1]][2:4])]
        print
        u'输入:[' + f1 + u']; 输出:[' + f2 + u']; 天干地支:[' + f3 + u']; 生肖:[' + f4 + ']'
    ...
    def helper_month_60_data_find(self, value, li):
    ...
    def helper_year_60_data_find(self, value, li):
    ...
    def helper_month_data_find(self, value, li):
        chinese_month_data_find = [(i, li.index(i)) for i in li if i <= value][-1]
        return [chinese_month_data_find[0], chinese_month_data_find[1], 1 + chinese_month_data_find[1],
                value - chinese_month_data_find[0] + 1]


    def helper_year_data_find(self, value, li):
        chinese_year_data_find = [(i, li.index(i)) for i in li if i <= value][-1]
        return [chinese_year_data_find[0], chinese_year_data_find[1], 1901 + chinese_year_data_find[1],
                value - chinese_year_data_find[0]]


    def change(self, chinese_year, chinese_month, chinese_day):
        CHANGE_MONTHS = {'00': 29, '01': 30, '10': 29, '11': 30, }
        chinese_years_days = self.helper_days_list()  # 旧历年，初一的天数
    ...
    def day_49(self, english_year, english_month, english_day):
        english_days = datetime.date(english_year, english_month, english_day) - datetime.date(1901, 1, 1)
        english_days = english_days.days  # 获取输入日期的天数
        chinese_year = '1900'
    ...
    def helper_verify_year(self, english_year):
        errors = 0
    ...
    def helper_verify_month(self, english_month):
        errors = 0
        english_year_error = 0
        if english_month < 1:
            errors = errors + 1
    ...
    def helper_verify_day(self, english_year, english_month, english_day):
    ...
    def helper_verify_day_60(self, chinese_year, chinese_month, chinese_day):
        errors = 0
        chinese_day_error = 0
